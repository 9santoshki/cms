...
const SuppliersPage = () => {
  ...
  return (
    <DashboardLayout title="Suppliers" description="Manage supplier profiles and inventory.">
      {/* Header */}
      <div className="suppliers-toolbar" style={{ display: 'flex', gap: 10, marginBottom: 12, alignItems: 'center', flexWrap: 'wrap' }}>
        <button onClick={() => setShowModal(true)} style={{ padding: '8px 16px', background: '#c19a6b', color: 'white', border: 'none', borderRadius: 6, fontSize: 12, fontWeight: 600, cursor: 'pointer', minHeight: '44px' }}>
          <i className="fas fa-plus"></i> Add Supplier
        </button>
        <button onClick={fetchData} style={{ padding: '8px 12px', background: 'white', border: '1px solid #e8d5c4', borderRadius: 6, fontSize: 12, cursor: 'pointer', color: '#c19a6b', minHeight: '44px' }}>
              ...
              {suppliers.map(s => (
                <tr key={s.id} style={{ borderBottom: '1px solid #f0f0f0' }}>
                  ...
                  <td style={{ padding: '8px 10px', textAlign: 'center' }}>
                    <span style={{ padding: '2px 6px', borderRadius: 4, fontSize: 12, fontWeight: 600,
                  ...
                  <td style={{ padding: '8px 10px', textAlign: 'center' }}>
                    <div style={{ display: 'flex', gap: 4, justifyContent: 'center', flexWrap: 'wrap' }}>
                      <button onClick={() => router.push(`/dashboard/suppliers/${s.id}`)} title="Manage variants"
                        style={{ padding: '6px 10px', borderRadius: 4, fontSize: 12, cursor: 'pointer', border: 'none', background: '#c19a6b', color: 'white', minHeight: '44px' }}>
                        <i className="fas fa-boxes"></i>
                      </button>
                      <button onClick={() => handleEdit(s)} title="Edit"
                        style={{ padding: '6px 10px', borderRadius: 4, fontSize: 12, cursor: 'pointer', border: '1px solid #e8d5c4', background: 'white', color: '#c19a6b', minHeight: '44px' }}>
                        <i className="fas fa-edit"></i>
                      </button>
                      <button onClick={() => handleDelete(s.id)} title="Delete"
                        style={{ padding: '6px 10px', borderRadius: 4, fontSize: 12, cursor: 'pointer', border: '1px solid #fee2e2', background: 'white', color: '#ef4444', minHeight: '44px' }}>

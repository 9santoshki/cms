  ...
  return (
    <DashboardLayout title="Appointments" description="Manage customer consultations.">
      ...
      <div style={{ background: 'white', borderRadius: 8, padding: '10px 16px', marginBottom: 12, border: '1px solid #e8d5c4', display: 'flex', gap: 12, alignItems: 'center' }}>
        ...
        <span style={{ fontSize: 12, color: '#666' }}>{filtered.length} appointments</span>
        <button onClick={fetchAppointments} style={{ marginLeft: 'auto', padding: '6px 12px', background: '#c19a6b', color: 'white', border: 'none', borderRadius: 6, fontSize: 12, cursor: 'pointer' }}>
              ...
              {filtered.map(a => (
                <tr key={a.id} style={{ borderBottom: '1px solid #f0f0f0' }}>
                  <td style={{ padding: '8px 10px' }}>
                    ...
                    {a.notes && <div style={{ fontSize: 11, color: '#888', maxWidth: 150, overflow: 'hidden', textOverflow: 'ellipsis' }}>{a.notes}</div>}
                  </td>
                  <td style={{ padding: '8px 10px', fontSize: 12, color: '#666' }}>
                    <div>{a.user_email || a.guest_email}</div>
                    {a.user_phone && <div style={{ fontSize: 11 }}>{a.user_phone}</div>}
                  </td>
                  <td style={{ padding: '8px 10px', fontSize: 12, color: '#666' }}>
                  ...
                  <td style={{ padding: '8px 10px', textAlign: 'center' }}>
                    <select value={a.status} onChange={e => updateStatus(a.id, e.target.value)}
                      style={{ padding: '4px 8px', border: '1px solid #e8d5c4', borderRadius: 4, fontSize: 11, cursor: 'pointer' }}>
                      <option value="pending">Pending</option>
                      <option value="confirmed">Confirmed</option>
                      <option value="completed">Completed</option>
                      <option value="cancelled">Cancelled</option>

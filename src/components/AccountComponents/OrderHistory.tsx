  ...
  return (
    <div className="bg-white rounded-xl shadow-md p-4">
      <div className="flex justify-between items-center mb-3 pb-2 border-b border-gray-200">
        ...
        <button className="text-amber-600 hover:text-amber-800 font-medium flex items-center text-sm">
          View All Orders
            ...
            {orders.length > 0 ? (
              orders.slice(0, 5).map((order: any) => (
                <tr key={order.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-3 py-2 whitespace-nowrap text-xs text-gray-500">{new Date(order.created_at).toLocaleDateString()}</td>
                  ...
                  <td className="px-3 py-2 whitespace-nowrap text-sm font-medium text-gray-900">${order.total_amount?.toFixed(2) || '0.00'}</td>
                ...
                <td colSpan={5} className="px-3 py-6 text-center">
                  <div className="flex flex-col items-center justify-center">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-10 w-10 text-gray-300 mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M16 11V7a4 4 0 00-8 0v4h8z" />
                    </svg>
                    ...
                    <button className="mt-2 px-3 py-1.5 bg-amber-600 text-white rounded-lg hover:bg-amber-700 transition-colors font-medium text-xs">
                      {t('browseProducts')}
                    </button>
                  </div>
                </td>
